...
param
(
    [parameter(Mandatory = $false)] [String] $FactoryName = "<ADF Name>",
    [parameter(Mandatory = $false)] [String] $ARMTemplateFilePath = -join ($(Get-Location), '\', $FactoryName, '\ARMTemplateForFactory.json'),
...
#Get dependants
ForEach ($Resource in $ADF.resources) {
    # | Where-Object {$_.type -ne "Microsoft.DataFactory/factories/triggers"})
